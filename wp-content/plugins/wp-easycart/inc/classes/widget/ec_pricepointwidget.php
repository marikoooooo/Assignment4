	...
	function widget($args, $instance){
		...
		if( isset( $_GET['menuid'] ) || isset( $_GET['submenuid'] ) || isset( $_GET['subsubmenuid'] ) ){
			...
			if( isset( $post_obj ) && isset( $post_obj->ID ) ){
				...
				if ( $menulevel1 && is_array( $menulevel1 ) && count( $menulevel1 ) > 0 ) {
					$level = 1;
					$menu_id = $menulevel1->menulevel1_id;
				} else if ( $menulevel2 && is_array( $menulevel2 ) && count( $menulevel2 ) > 0 ) {
					$level = 2;
					$menu_id = $menulevel2->menulevel2_id;
				} else if ( $menulevel3 && is_array( $menulevel3 ) && count( $menulevel3 ) > 0 ) {
					$level = 3;
