...
<?php wp_easycart_admin_verification( )->print_nonce_field( 'wp_easycart_nonce', 'wp-easycart-menulevel3-details' ); ?>
...
<?php
	if( isset( $_GET['menulevel2_id'] ) ){
		...
		global $wpdb;
		$menulevel2_id = $wpdb->get_var( $wpdb->prepare( "SELECT ec_menulevel3.menulevel2_id FROM ec_menulevel3 WHERE ec_menulevel3.menulevel3_id = %d", (int) $_GET['menulevel3_id'] ) );
	}
?>
<input type="hidden" name="menulevel2_id" value="<?php echo esc_attr( $menulevel2_id ); ?>" />	
<input type="hidden" name="menulevel3_id" value="<?php echo esc_attr( $this->menulevel3->menulevel3_id ); ?>" />
        ...
        <div class="ec_admin_flex_row">
            <div class="ec_admin_list_line_item ec_admin_col_12 ec_admin_col_first">
            	<div class="ec_admin_settings_label">
                    ...
                    <span><?php if( isset( $_GET['ec_admin_form_action'] ) && $_GET['ec_admin_form_action'] == 'add-new-menulevel3' ){ esc_attr_e( 'ADD NEW SUB-SUB-MENU', 'wp-easycart' ); }else{ esc_attr_e( 'EDIT SUB-SUB-MENU', 'wp-easycart' ); } ?></span>
                    <div class="ec_page_title_button_wrap">
                        ...
                        <a href="<?php echo esc_attr( $this->action ); ?>&menulevel2_id=<?php echo esc_attr( $menulevel2_id ); ?>" class="ec_page_title_button"><?php esc_attr_e( 'Back to Sub-Sub-Menu', 'wp-easycart' ); ?></a>
                        <?php if ( is_plugin_active( 'wordpress-seo/wp-seo.php' ) || is_plugin_active( 'wordpress-seo-premium/wp-seo-premium.php' ) ){ ?>
                        <a href="post.php?post=<?php echo esc_attr( $this->menulevel3->post_id ); ?>&action=edit" target="_blank" class="ec_page_title_button"><?php esc_attr_e( 'Edit Yoast SEO', 'wp-easycart' ); ?></a>
                        <?php } ?>
